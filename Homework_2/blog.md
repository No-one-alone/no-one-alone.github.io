...
``` bash
    git branch 
```
Adding a name of a branch that doesn't exist result in the creation of a new branch.

``` bash
    git branch featurebranchname
```

One could then add, commit, and push on this branch just we previously did on master.
...
Note, that one must be on the branch one wishes to merge into instead of the branch be merged into it.

### Part 2: Planning and Design

I wanted to keep it simple so I decided on designing as webpage that would check basic number properties between two numbers supplied by the user and return the results as part of a table. To help with this process, I created the following sketch of the webpage here.
...
As you can see, I decided to keep the HTML, CSS, and Javascript/jQuery code all in separate files for both better organization as well as to be in line with more modern software design practices.
...
Then a bit afterward, I coded the elements that would take number input from the users as seen here.
...
In addition, I created the checkbox section where the user could select checkbox options as follows.
...
Finally, the Output section and heading were done in this manner.
...
Now, in preparation for the Javascript/jQuery scripting portion. I placed div element with an "error" id just below the greeting html where an error message would be inserted and subsequently removed dependent on the user input and action.
...
Also, I used the div element with the id of "totaltarget" seen here as where the results of valid user inputs would be inserted into the html as a table.
...
And to actually add an element to the webpage such at the element with the "target" class, this jQuery code proved useful provided a string composed of html has been prepared before hand as the input.

```javascript
 $(".target").append(rowHTML); // This uses jQuery to append the constructed html element of answers to the web page.
```
Finally, to modfiy an HTML element in place such as with resetting the input boxes to containing the number 1 again after each click on analyze, the following proved most useful.
...
As for straight Javascript, I elected to refacter the code to use it as little as possible while trying to use jQuery where ever I could. This not only created a script more likely to not break on different browsers, but also, dramatically shortened the overall program!

Indeed, what javascript I did end up using was not significantly different from what one may have seen with working with the java language. The most pure javascript use can be seen here as far as how functions, conditionals
...
![Webpages](web_page_better_2.PNG)
